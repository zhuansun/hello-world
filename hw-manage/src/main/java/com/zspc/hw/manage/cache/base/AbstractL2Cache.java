...
import com.zspc.hw.common.util.ApplicationContextUtils;
...
public abstract class AbstractL2Cache<K, V> implements L2Cache<K, V> {
    ...
    private final long l2TTLSeconds = l2TTLSeconds();
    ...
    private final HwRedisClient redisCache = initRedisCache();
     ...
     * @return long 过期时间
     */
    protected abstract long l2TTLSeconds();
    ...
    private Cache<K, Optional<V>> initCaffeine() {
        if (l1TTLSeconds <= 0L || l1MaxSize <= 0) {
            throw BusinessException.get("初始化L1(Caffeine)参数不可小于0");
        }
     ...
     * @return RedisCache
     */
    private HwRedisClient initRedisCache() {
        // 未启用不处理
        if (!isEnableL2) {
            return null;
        }
        ...
        HwRedisClient redisCache = ApplicationContextUtils.getBean(HwRedisClient.class);
        if (redisCache == null) {
            isEnableL2 = false;
        ...
        // 初始化校验
        if (l2TTLSeconds <= 0) {
            throw BusinessException.get("初始化L2(RedisCache)参数不可小于0");
        }
        return redisCache;
    }
    ...
    private void l2Put(K k, Optional<V> v) {
        ...
        if (v.isPresent()) {
            redisCache.set(l2Key(k), v.get(), l2TTLSeconds);
        } else if (isCacheNull) {
            redisCache.set(l2Key(k), NULLABLE, l2TTLSeconds);
        }
    }
